...
if (rv == CKR_OK) {
    ...
    // CKA_NEVER_EXTRACTABLE = FALSE（因为设置EXTRACTABLE=FALSE）
}
```
...
CKA_NEVER_EXTRACTABLE = FALSE（如果CKA_EXTRACTABLE = FALSE）
→ 从创建开始就不可导出，永不改变
```
...
CKM_RSA_X9_31_KEY_PAIR_GEN RSA密钥对（X9.31标准）

CKM_DSA_KEY_PAIR_GEN       DSA密钥对
CKM_DH_PKCS_KEY_PAIR_GEN   DH密钥对

CKM_EC_KEY_PAIR_GEN        EC密钥对（通用）
CKM_ECDSA_KEY_PAIR_GEN     ECDSA密钥对
CKM_ECDH_KEY_PAIR_GEN      ECDH密钥对

国密密钥：
CKM_SM2_KEY_PAIR_GEN       SM2密钥对
```
